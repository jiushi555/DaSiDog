...
public class Add_Gz extends Activity {
	private TextView add_btn;
	private EditText add_edt;
	private TextView back;
	private String add_txt;
	private int user_id;
	private String url = Welcome.URL + "/dasidog/add_gz.php";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.add_gz);
		Intent it = super.getIntent();
		user_id = it.getIntExtra("user_id", 0);
		add_btn = (TextView) findViewById(R.id.gz_publish);
		add_edt = (EditText) findViewById(R.id.gz_publish_text);
		back = (TextView) findViewById(R.id.gz_btn_left);
		back.setOnClickListener(new backOnClick());
		add_btn.setOnClickListener(new addOnClick());
		add_img_btn = (ImageView) findViewById(R.id.gz_add_img);
	...
	private class addImgOnClick implements OnClickListener {
		...
		public void onClick(View v) {
			LayoutInflater inflater = LayoutInflater.from(Add_Gz.this);
			...
			popwin.showAtLocation(Add_Gz.this.add_img_btn, Gravity.CENTER, 0, 0);
	...
	private void setPic() {
		...
		Add_Gz.this.bitmap = rotatedBMP;
		Add_Gz.this.bitmapX = rotatedBMPX;
	}
	...
	class addOnClick implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (add_txt.equals("")) {
				Toast.makeText(Add_Gz.this, "请输入内容", Toast.LENGTH_SHORT).show();
			} else {
				if (bitmap != null) {
	...
	class SignupUser extends AsyncTask<String, String, String> {
		protected void onPreExecute() {
			super.onPreExecute();
			progressDialog = CustomProgressDialog.createDialog(Add_Gz.this);
			progressDialog.setMessage("发布中...");
			Add_Gz.this.progressDialog.show();
		}
		...
		protected String doInBackground(String... args) {
			...
			params.add(new BasicNameValuePair("imgL", img_fileL));
			params.add(new BasicNameValuePair("imgX", img_fileX));
			JSONObject json = jsonParser.makeHttpRequest(url, "POST", params,
					Add_Gz.this);
			Log.d("Create Response", json.toString());
			try {
				Add_Gz.this.success = json.getInt("success");
		...
		protected void onPostExecute(String file_url) {
			...
			if (success == 2) {
				Toast.makeText(Add_Gz.this, "发布失败，刷新试试？", Toast.LENGTH_SHORT)
						.show();
			} else {
				refresh();
				Toast.makeText(Add_Gz.this, "发布成功！", Toast.LENGTH_SHORT).show();
			}
	...
	private void refresh() {
		Add_Gz.this.finish();
		Intent it = new Intent(Add_Gz.this, Sy_Tab.class);
		it.putExtra("user_id", user_id);
		it.putExtra("re_type", "yes");
		it.putExtra("page", "1");
		Add_Gz.this.startActivity(it);
	}
}
