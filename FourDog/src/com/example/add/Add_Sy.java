...
import com.example.add.Add_Sy.SignupUser;
...
public class Add_Sy extends Activity {
	private int id;
	private RadioGroup add_to;
	private RadioButton add_to_gz;
	private RadioButton add_to_ky;
	private int add_to_id = 0;
	private TextView txt_gz;
	private TextView txt_ky;
	private TextView public_txt;
	private TextView back;
	private String url = Welcome.URL + "/dasidog/add.php";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.add_sy);
		Intent it = super.getIntent();
		id = it.getIntExtra("user_id", 0);
		add_to = (RadioGroup) findViewById(R.id.add_to);
		add_to_gz = (RadioButton) findViewById(R.id.gz);
		add_to_ky = (RadioButton) findViewById(R.id.ky);
		txt_gz = (TextView) findViewById(R.id.txt_gz);
		txt_ky = (TextView) findViewById(R.id.txt_ky);
		public_txt = (TextView) findViewById(R.id.publish);
		back = (TextView) findViewById(R.id.btn_left);
		nr = (EditText) findViewById(R.id.publish_text);
		add_img_btn = (ImageView) findViewById(R.id.add_img);
		add_img_btn.setOnClickListener(new addImgOnClick());
		super.registerForContextMenu(add_img_btn);
		back.setOnClickListener(new backOnClick());
		public_txt.setOnClickListener(new publicOnClick());
		add_to.setOnCheckedChangeListener(new addtoListener());
	}
	...
	private class addImgOnClick implements OnClickListener {
		...
		public void onClick(View v) {
			LayoutInflater inflater = LayoutInflater.from(Add_Sy.this);
			...
			popwin.showAtLocation(Add_Sy.this.add_img_btn, Gravity.CENTER, 0, 0);
	...
	private void setPic() {
		...
		Add_Sy.this.bitmap = rotatedBMP;
		Add_Sy.this.bitmapX = rotatedBMPX;
	}
	/**
	 * 压缩详细图片
	 */
...
//      return compressBmpFromBmp(bitmap);//原来的方法调用了这个方法企图进行二次压缩  
	...
	class publicOnClick implements OnClickListener {

		@Override
		public void onClick(View v) {
			public_nr = nr.getText().toString();
			if (public_nr.equals("")) {
				Toast.makeText(Add_Sy.this, "请输入内容", Toast.LENGTH_SHORT).show();
			} else if (add_to_id == 0) {
				Toast.makeText(Add_Sy.this, "请选择发布的地方", Toast.LENGTH_SHORT)
						.show();
			} else {
				if (bitmap != null) {
	...
	class SignupUser extends AsyncTask<String, String, String> {
		protected void onPreExecute() {
			super.onPreExecute();
			progressDialog = CustomProgressDialog.createDialog(Add_Sy.this);
			progressDialog.setMessage("发布中...");
			Add_Sy.this.progressDialog.show();
		}

		protected String doInBackground(String... args) {

			List<NameValuePair> params = new ArrayList<NameValuePair>();
			params.add(new BasicNameValuePair("nr", public_nr));
			params.add(new BasicNameValuePair("add_to_id", String
					.valueOf(add_to_id)));
			params.add(new BasicNameValuePair("writer", String.valueOf(id)));
			params.add(new BasicNameValuePair("img", String.valueOf(img_fileL)));
			params.add(new BasicNameValuePair("imgX", String.valueOf(img_fileX)));
			JSONObject json = jsonParser.makeHttpRequest(url, "POST", params,Add_Sy.this);
			Log.d("Create Response", json.toString());
			try {
				Add_Sy.this.success = json.getInt("success");
		...
		protected void onPostExecute(String file_url) {
			...
			if (success == 2) {
				Toast.makeText(Add_Sy.this, "发布失败，刷新试试？", Toast.LENGTH_SHORT)
						.show();
			} else {
				refresh();
				Toast.makeText(Add_Sy.this, "发布成功！", Toast.LENGTH_SHORT).show();
				Add_Sy.this.finish();
			}
	...
	class addtoListener implements OnCheckedChangeListener {

		@Override
		public void onCheckedChanged(RadioGroup group, int checkedId) {
			if (add_to_gz.getId() == checkedId) {
				add_to_id = 1;
				txt_gz.setTextColor(getResources().getColor(R.color.res_cor3));
				txt_ky.setTextColor(getResources().getColor(R.color.text_hint));
				
			}
			if (add_to_ky.getId() == checkedId) {
				add_to_id = 2;
				txt_ky.setTextColor(getResources().getColor(R.color.res_cor3));
				txt_gz.setTextColor(getResources().getColor(R.color.text_hint));
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		menu.add(Menu.NONE, 1, Menu.NONE, "相册");
		menu.add(Menu.NONE, 2, Menu.NONE, "拍照");
	}
	private void refresh(){
		Add_Sy.this.finish();
		Intent it=new Intent(Add_Sy.this,Sy_Tab.class);
		it.putExtra("user_id", id);
		it.putExtra("re_type", "yes");
		it.putExtra("page", "0");
		Add_Sy.this.startActivity(it);
	}
}
