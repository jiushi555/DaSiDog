...
public class SampleFragment extends Fragment implements OnItemClickListener {
	...
	private String url = Welcome.URL + "/dasidog/ky_pl_jz.php";
	private int success;
	...
	private String url_pl = Welcome.URL + "/dasidog/ky_pl_fb.php";
	private Context context;
	private String re_type;
	private Map<String, String> map;

	public SampleFragment(int mPosition, int headH,Context context,String re_type,Map map) {
	...
	class Gz_Pl_refresh extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... args) {
			...
			params.add(new BasicNameValuePair("ky_id", String
			...
			JSONObject json = jsonParser.makeHttpRequest(url, "POST", params,context);
			Log.d("Create Response", json.toString());
			try {
				SampleFragment.this.success = json.getInt("success");
				if (success == 1) {
					data = json.getJSONArray("ky_pl");
	...
	private class PlOnItemClick implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
			...
			SampleFragment.this.pl_to_name=mgs.getWriterName(arg2 - 1, arg1, mListView);
	...
	private class OnScrollListenerlmpl implements OnScrollListener {
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			...
			if (SampleFragment.this.lastItem == SampleFragment.this.mgs
				...
				System.out
						.println(SampleFragment.this.mgs.getCount());
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		if(item.getItemId()==2){
			...
			clip.setText(pl_nr);
			Toast.makeText(getActivity(), "内容已复制到剪切板", Toast.LENGTH_SHORT).show();
		}
		if(item.getItemId()==3){
	...
	class Gz_Pl_ extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... args) {
			...
			JSONObject json = jsonParser
					.makeHttpRequest(url_pl, "POST", params,context);
			Log.d("Create Response", json.toString());
			try {
				SampleFragment.this.success = json.getInt("success");
	...
	private void refresh() {
		...
		Intent it_to_a = new Intent(context, KyArticle.class);
		...
		it_to_a.putExtra("user_tx", map.get("user_tx_url"));
