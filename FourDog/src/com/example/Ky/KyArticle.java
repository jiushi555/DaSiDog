...
public class KyArticle extends FragmentActivity implements
TabBaseFramentListener {
	...
	private String url_pl_ = Welcome.URL + "/dasidog/ky_pl_fb.php";
	private int success;
...
//	顶部停靠
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		user_tx=it.getStringExtra("user_tx");
	...
	private class FaceOnClick implements OnClickListener {
		...
		public void onClick(View v) {
			if (null == mFaceHelper) {
				...
				mFaceHelper = new SelectFaceHelper(KyArticle.this,
						addFaceToolView);
			...
			if (isVisbilityFace) {
				...
				hideInputManager(KyArticle.this);
			}
		}
	...
	private class ImgOnClick implements OnClickListener{
		...
		@Override
		public void onClick(View v) {
			Intent it=new Intent(KyArticle.this,ImageX.class);
			it.putExtra("pathX", pathX);
			KyArticle.this.startActivity(it);
		}
		
	}
	private void init(int type,int id,String name,String nr_id,int line) {
		frament = new SampleFragment(type, headH,KyArticle.this,re_type,map_to);
	...
	private class plOnClick implements OnClickListener {

		@Override
		public void onClick(View v) {
			pl_txt = pl_edt.getText().toString();
			if (pl_txt.equals("")) {
				Toast.makeText(KyArticle.this, "请输入评论！", Toast.LENGTH_SHORT)
						.show();
			} else {
				new KyArticle.Sy_pl().execute();
			}
	...
	private class Sy_pl extends AsyncTask<String, String, String> {
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = CustomProgressDialog.createDialog(KyArticle.this);
		...
		protected String doInBackground(String... args) {
			...
			params_.add(new BasicNameValuePair("from", String.valueOf(user_id)));
			params_.add(new BasicNameValuePair("to", String.valueOf(KyArticle.this.writer_id)));
			params_.add(new BasicNameValuePair("txt_id", String.valueOf(txt_id)));
			params_.add(new BasicNameValuePair("pl_nr", pl_txt));
			JSONObject json = jsonParser
					.makeHttpRequest(url_pl_, "POST", params_,KyArticle.this);
			Log.d("Create Response", json.toString());
			try {
				KyArticle.this.success = json.getInt("success");
		...
		protected void onPostExecute(String file_url) {
			...
			if (KyArticle.this.success == 1) {
				refresh();
				Toast.makeText(KyArticle.this, "评论成功", Toast.LENGTH_SHORT)
						.show();

			}
			if (KyArticle.this.success == 2) {
				Toast.makeText(KyArticle.this, "评论失败，刷新试试？", Toast.LENGTH_SHORT)
						.show();
			}
	...
	private void refresh() {
		...
		finish();
		Intent it_to_a = new Intent(KyArticle.this, KyArticle.class);
		...
		it_to_a.putExtra("user_tx",user_tx);
