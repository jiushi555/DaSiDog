...
import com.example.Sy.SampleFragment;
import com.example.Sy.TabBaseFramentListener;
...
public class SyArticle extends FragmentActivity implements
	...
	private String url_pl = Welcome.URL + "/dasidog/sy_pl_fb.php";
	private int success;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tx_url = it.getStringExtra("tx_url");
		...
		// 顶部停靠
		initAllViews();
		init(now, type, user_id, writer_name, txt_id, line);
	...
	private class FaceOnClick implements OnClickListener {
		...
		public void onClick(View v) {
			if (null == mFaceHelper) {
				...
				mFaceHelper = new SelectFaceHelper(SyArticle.this,
						addFaceToolView);
			...
			if (isVisbilityFace) {
				...
				hideInputManager(SyArticle.this);
			}
		}
	...
	private class ImgOnClick implements OnClickListener {
		...
		@Override
		public void onClick(View v) {
			Intent it = new Intent(SyArticle.this, ImageX.class);
			it.putExtra("pathX", pathX);
			SyArticle.this.startActivity(it);

		}

	}

	private void init(String now, int type, int id, String name, String nr_id,
			int line) {
		frament = new SampleFragment(now, type, headH, SyArticle.this, re_type,
				map_to);
	...
	class plOnClick implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (pl_txt.equals("")) {
				Toast.makeText(SyArticle.this, "请输入评论！", Toast.LENGTH_SHORT)
						.show();
			} else {
				new SyArticle.Sy_pl().execute();
			}
	...
	class Sy_pl extends AsyncTask<String, String, String> {
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = CustomProgressDialog.createDialog(SyArticle.this);
		...
		protected String doInBackground(String... args) {
			...
			params.add(new BasicNameValuePair("now", now));
			JSONObject json = jsonParser.makeHttpRequest(url_pl, "POST",
					params, SyArticle.this);
			Log.d("Create Response", json.toString());
			try {
				SyArticle.this.success = json.getInt("success");
		...
		protected void onPostExecute(String file_url) {
			...
			if (SyArticle.this.success == 1) {
				refresh();
				Toast.makeText(SyArticle.this, "评论成功", Toast.LENGTH_SHORT)
						.show();

			}
			if (SyArticle.this.success == 2) {
				Toast.makeText(SyArticle.this, "评论失败，刷新试试？", Toast.LENGTH_SHORT)
						.show();
			}
	...
	private void refresh() {
		...
		finish();
		Intent it_to_a = new Intent(SyArticle.this, SyArticle.class);
		...
		it_to_a.putExtra("tx_url", tx_url);
